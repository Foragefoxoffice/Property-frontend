...
export default function FloorRange() {
  ...
  // ✅ Fetch Floor Ranges
  const fetchFloorRanges = async () => {
    try {
      setLoading(true);
      const res = await getAllFloorRanges();
  ...
  // ✅ Pagination
  const totalRows = floorRanges.length;
  const totalPages = Math.max(1, Math.ceil(totalRows / rowsPerPage));
  const startIndex = (currentPage - 1) * rowsPerPage;
  const endIndex = Math.min(startIndex + rowsPerPage, totalRows);
  const visibleData = floorRanges.slice(startIndex, endIndex);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (msg?.toLowerCase().includes("floor range with this name already exists") && isVI) {
        msg = "Khoảng tầng với tên này đã tồn tại";
      }
    ...
    <div className="p-8 min-h-screen relative">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        >
          ...
          {isVI ? "Thêm khoảng tầng" : "Add Floor Range"}
        </button>
      </div>
          ...
          <table className="w-full text-sm border-collapse">
            <thead className="bg-gray-50 text-gray-700">
              <tr>
                ...
                <th className="px-6 py-3 font-medium text-left">
                  {isVI ? "Tên khoảng tầng" : "Floor Range Name"}
                </th>
                <th className="px-6 py-3 font-medium text-left">
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50" onMouseDown={(e) => { if (e.target === e.currentTarget) { setDeleteConfirm({ show: false, id: null }); } }}>
          <div className="bg-white rounded-2xl p-6 w-full max-w-sm shadow-lg">
            ...
            <p className="text-sm text-gray-600 mb-5">
              {isVI
                ? "Bạn có chắc chắn muốn xóa khoảng tầng này? Hành động này không thể hoàn tác."
                : "Are you sure you want to delete this floor range? This action cannot be undone."}
            </p>
                ...
                <>
                  <div>
                    ...
                    <input
                      type="text"
                      name="name_en"
                      placeholder="Type here (e.g., G+1 to G+5)"
                      value={form.name_en}
                ...
                <>
                  <div>
                    ...
                    <input
                      type="text"
                      name="name_vi"
                      placeholder="Nhập tại đây (ví dụ: Tầng trệt đến 5 tầng)"
                      value={form.name_vi}
                      onChange={handleChange}
                      className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-1 focus:outline-none"
                    />
