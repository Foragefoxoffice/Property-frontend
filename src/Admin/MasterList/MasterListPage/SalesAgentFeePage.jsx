...
export default function SalesAgentFeePage() {
  ...
  const fetchRecords = async () => {
    try {
      setLoading(true);
      const res = await getAllSalesAgentFees();
      setRecords(res.data.data || []);
    } catch {
      CommonToaster(
        isVI
          ? "Không thể tải danh sách phí đại lý."
          : "Failed to load Sales Agent Fee list.",
        "error"
      );
    } finally {
      setLoading(false);
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (editingRecord) {
        await updateSalesAgentFee(editingRecord._id, form);
        CommonToaster(
          isVI ? "Cập nhật thành công!" : "Sales Agent Fee updated successfully!",
          "success"
        );
      } else {
        await createSalesAgentFee(form);
        CommonToaster(
          isVI ? "Thêm mới thành công!" : "Sales Agent Fee created successfully!",
          "success"
        );
      }
      ...
      if (
        ...
        CommonToaster(
          isVI
            ? "Phí đại lý này đã tồn tại."
            : "This Sales Agent Fee name already exists.",
          "error"
        );
        return;
      }
  ...
  const handleDelete = async () => {
    try {
      await deleteSalesAgentFee(deleteConfirm.id);
      CommonToaster(
  ...
  const handleToggleStatus = async (record) => {
    ...
    try {
      await updateSalesAgentFee(record._id, { status: newStatus });
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-2xl font-semibold text-gray-900">
            {isVI ? "Phí đại lý bán hàng" : "Sales Agent Fee"}
          </h2>
        </div>
        ...
        >
          ...
          {isVI ? "Thêm phí đại lý" : "Add Sales Agent Fee"}
        </button>
      </div>
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg overflow-hidden">
            ...
            <div className="flex justify-between items-center px-6 py-4">
              <h2 className="text-lg font-medium">
                {editingRecord
                  ? activeLang === "EN"
                    ? "Edit Sales Agent Fee"
                    : "Chỉnh sửa phí đại lý"
                  : activeLang === "EN"
                    ? "New Sales Agent Fee"
                    : "Thêm phí đại lý"}
              </h2>
            ...
            <div className="p-6 space-y-5">
              {activeLang === "EN" ? (
                <>
                  <div>
                    <label className="block text-sm font-medium mb-1">
                      Sales Agent Fee <span className="text-red-500">*</span>
                    </label>
                    <input
                      name="name_en"
                      value={form.name_en}
                ...
                <>
                  <div>
                    <label className="block text-sm font-medium mb-1">
                      Phí đại lý <span className="text-red-500">*</span>
                    </label>
                    <input
                      name="name_vi"
                      value={form.name_vi}
