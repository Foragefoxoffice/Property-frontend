...
export default function LegalDocumentPage({ goBack }) {
  ...
  const fetchRecords = async () => {
    try {
      setLoading(true);
      const res = await getAllLegalDocuments();
      setRecords(res.data.data || []);
    } catch {
      CommonToaster(
        isVI ? "Không thể tải tài liệu pháp lý." : "Failed to load legal documents.",
        "error"
      );
    } finally {
      setLoading(false);
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (editingRecord) {
        await updateLegalDocument(editingRecord._id, form);
        CommonToaster(
        ...
        CommonToaster(
          isVI ? "Thêm mới thành công!" : "Added successfully!",
          "success"
        );
      }
  ...
  const handleToggleStatus = async (record) => {
    ...
    try {
      await updateLegalDocument(record._id, { status: newStatus });
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-2xl font-semibold text-gray-900">
            {isVI ? "Tài liệu pháp lý" : "Legal Documents"}
          </h2>
        </div>
          ...
          <table className="w-full text-sm border-collapse">
            <thead className="bg-gray-50 text-gray-700">
              <tr>
                ...
                <th className="px-6 py-3 text-left">{isVI ? "Tên tài liệu" : "Legal Document"}</th>
