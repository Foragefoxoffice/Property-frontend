...
export default function PrivacyPolicyPage() {
    ...
    useEffect(() => {
        ...
        // 1. Update Title
        const metaTitle = pageData[`privacyPolicySeoMetaTitle_${langKey}`] ||
            (langKey === 'en' ? 'Privacy Policy' : 'Chính Sách Bảo Mật');
        ...
        // 2. Meta Description
        const metaDesc = pageData[`privacyPolicySeoMetaDescription_${langKey}`] || '';
        updateMeta('description', metaDesc);

        // 3. Keywords
        const keywords = pageData[`privacyPolicySeoMetaKeywords_${langKey}`];
        ...
        const canonicalUrl = pageData[`privacyPolicySeoCanonicalUrl_${langKey}`];
        if (canonicalUrl) {
        ...
        // 5. Open Graph Tags
        const ogTitle = pageData[`privacyPolicySeoOgTitle_${langKey}`] || metaTitle;
        const ogDesc = pageData[`privacyPolicySeoOgDescription_${langKey}`] || metaDesc;
        ...
        if (Array.isArray(ogImages) && ogImages.length > 0) {
            ogImages.forEach(imgUrl => {
                ...
                if (!imgUrl.includes('/') && !imgUrl.startsWith('http')) {
                    rawUrl = `/uploads/privacypolicypage/${imgUrl}`;
                }
        ...
        // 7. Robots / Indexing
        const allowIndexing = pageData.privacyPolicySeoAllowIndexing !== false; // Default true
